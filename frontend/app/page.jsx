...
export default function App() {
  ...
  const handleSummarize = useCallback(async () => {
    ...
    try {
      ...
      const obj = {};
      (Array.isArray(data.actionItems) ? data.actionItems : []).forEach((item) => {
  ...
  const hasResults = summary.trim().length > 0 || Object.keys(assigned).length > 0;
  ...
  return (
    <div className="relative min-h-screen overflow-hidden bg-black text-cyan-100">
      ...
      {/* Neon frame lines */}
      <motion.div initial={{ scaleX: 0 }} animate={{ scaleX: 1 }} transition={{ duration: 0.8 }} className="absolute left-0 right-0 top-0 h-[2px] origin-left bg-gradient-to-r from-fuchsia-500 via-cyan-400 to-fuchsia-500" />
      <motion.div initial={{ scaleY: 0 }} animate={{ scaleY: 1 }} transition={{ duration: 0.8, delay: 0.1 }} className="absolute bottom-0 left-0 top-0 w-[2px] origin-top bg-gradient-to-b from-cyan-400 via-fuchsia-500 to-cyan-400" />
      ...
      <main className="relative mx-auto flex min-h-screen max-w-5xl flex-col items-center justify-center px-4 py-10">
        ...
        <motion.h1 className="relative mb-6 text-center text-4xl font-extrabold tracking-tight sm:text-5xl">
          <motion.span
            ...
            transition={{
              ...
              repeat: Infinity,    // infinite flicker
              repeatType: "loop",
              duration: 1.2,       // speed of flicker sequence
            }}
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="rounded-xl border border-fuchsia-500/20 bg-black/40 p-4 shadow-[0_0_30px_rgba(217,70,239,.15)]">
              <div className="mb-2 flex items-center justify-between">
                <h2 className="text-xs font-semibold uppercase tracking-wider text-fuchsia-300/90">Input</h2>
                <div className="flex items-center gap-2 text-[11px]">
                  <button
                    onClick={() => setText("Alice: Prepare the deck.\nBob: Collect metrics.\nAlice: Ship v1 by Friday.\nCara: Draft release notes.")}
                  ...
                  <label className="cursor-pointer rounded-md border border-fuchsia-500/30 bg-fuchsia-500/10 px-2.5 py-1 text-fuchsia-200 hover:bg-fuchsia-500/20">
                    <input type="file" accept=".txt" className="hidden" onChange={async (e) => { const f = e.target.files?.[0]; if (!f) return; const txt = await f.text(); setText(txt); }} />
                    .txt
                  </label>
                </div>
              </div>
              ...
              <div className="mt-2 flex items-center justify-between text-[11px] text-cyan-300/70">
                ...
                <span className="rounded-full border border-cyan-500/30 bg-cyan-500/10 px-2 py-0.5">Ctrl/⌘+Enter</span>
              </div>
            </div>
            ...
            <div className="rounded-xl border border-cyan-500/20 bg-black/40 p-4 shadow-[0_0_30px_rgba(34,211,238,.15)]">
              <div className="mb-2 flex items-center justify-between">
                <h2 className="text-xs font-semibold uppercase tracking-wider text-cyan-300/90">Results</h2>
                <button
                  disabled={!hasResults}
                  onClick={async () => {
                    const out = [];
                    ...
                    if (keys.length) {
                      out.push("\nAction Items:");
                      keys.forEach((k) => assigned[k].forEach((t) => out.push(`- ${k}: ${t}`)));
                    }
                    const final = out.join("\n");
                    if (final) await navigator.clipboard.writeText(final);
                  }}
              ...
              <div className="grid gap-3">
                <AnimatePresence mode="popLayout">
                  {summary ? (
                    ...
                    >
                      <div className="mb-1 text-sm font-semibold text-fuchsia-300">Summary</div>
                      <p className="whitespace-pre-wrap text-sm text-cyan-100/90">{summary}</p>
                    </motion.div>
                  ) : (
                    !loading && (
                      <motion.div
                ...
                <AnimatePresence>
                  {Object.keys(assigned).length > 0 && (
                    ...
                    >
                      <div className="mb-2 text-sm font-semibold text-cyan-300">Action Items</div>
                      <div className="grid gap-2">
                        {Object.entries(assigned).map(([person, tasks]) => (
                          <motion.div key={person} variants={item} className="rounded-md border border-fuchsia-500/30 bg-black/60 p-2">
                            <div className="mb-1 text-[13px] font-semibold text-fuchsia-300">{person}</div>
                            <ul className="list-disc list-inside space-y-1 text-[13px] text-cyan-100/90">
                              {tasks.map((t, i) => (
                                <li key={i} className="hover:text-cyan-300/90 transition-colors">{t}</li>
                              ))}
                            </ul>
                          </motion.div>
                        ))}
                ...
                {loading && (
                  <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="grid place-items-center rounded-lg border border-cyan-500/30 bg-black/60 p-6">
                    <motion.div
                      animate={{ rotate: 360 }}
                      transition={{ repeat: Infinity, duration: 1, ease: "linear" }}
                      className="h-5 w-5 rounded-full border-2 border-cyan-400/60 border-t-transparent"
                    />
                    <p className="mt-2 text-xs text-cyan-300/80">Synthesizing…</p>
                  </motion.div>
                )}

                {error && <div className="text-sm text-fuchsia-300/90">{error}</div>}
              </div>
            </div>
          </div>

          {/* Controls centered under grid */}
          <div className="mt-4 flex items-center justify-center gap-2">
            <button onClick={handleClear} className="rounded-xl border border-fuchsia-500/30 bg-fuchsia-500/10 px-4 py-2 text-sm text-fuchsia-200 hover:bg-fuchsia-500/20">
              Clear
            </button>
            <button
        ...
        {/* footer */}
        <div className="mt-6 text-center text-[11px] text-cyan-300/60">
          Neon grid • Tailwind • Framer Motion
        </div>
      </main>
    </div>
  );
